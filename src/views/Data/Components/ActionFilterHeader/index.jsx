...
const ActionFilterHeader = ({ title, label }) => {
  ...
  const valueFilter = filterTableDataPage[`${title}`];
  const isActiveFilter =
    !isEmpty(valueFilter) ||
    isDate(valueFilter) ||
    (typeof valueFilter === 'boolean' && valueFilter) ||
    (title === 'interactiveDate' && filterTableDataPage.startInteractiveDate) ||
    (title === 'firstAssignDate' && filterTableDataPage.startFirstAssignDate) ||
    (title === 'assignDate' && filterTableDataPage.startAssignDate);
