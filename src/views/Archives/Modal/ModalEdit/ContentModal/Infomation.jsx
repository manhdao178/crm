...
const Infomation = ({ data = {}, role = '', refetch = () => {}, toggle, toggleAddCalendar, item }) => {
  //! State
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const archives = await queryClient.refetchQueries([queryKeys.archives]);
      await Promise.all([leadDetails, leads, leadsAppointment, leadsManage, archives]);
      return;
    } catch (error) {
          ...
          <Form>
            <Grid container spacing={2}>
              ...
              <Grid item xs={6}>
                <Field
                  disabled
                  label={t('common:data_telesale')}
                  name="telesale"
                  LabelColorPrimary
                  placeholder={t('common:data_telesale')}
                  component={InputField}
                />
              </Grid>
              <Grid item xs={6}>
